...
if [ "$(pidof crond | wc -l)" -eq "0" ]; then
	$BB nohup /system/xbin/crond -c /var/spool/cron/crontabs/ > /data/.shift/cron.txt &
	PIDOFCRON=$(pidof crond);
	echo "-900" > /proc/"$PIDOFCRON"/oom_score_adj;
fi;

$BB sh /res/crontab_service/dm_job.sh "3:00" "/sbin/busybox2 sh /data/crontab/cron-scripts/database_optimizing.sh"
$BB sh /res/crontab_service/dm_job.sh "4:00" "/sbin/busybox2 sh /data/crontab/cron-scripts/clear-file-cache.sh"
